...
function Drum() {
 ...
 const [nickname, setNickname] = useState("");
  ...
  const fetchVideoPosts = async () => {
    setLoading(true);
    try {
      const res = await instance.get(`/posts5_1`);
  ...
  const handleSubmit = async e => {
    ...
    let newPost;
    if (boardType === String("드럼 게시판 연주영상")) {
      newPost = {
        title,
        link: youtubeLink,
        user_id: "이름",
    ...
    try {
      if (isEditing) {
        await instance.put(`/posts5/${editingPostId}`, newPost);
      } else {
        const endpoint =
          boardType === "드럼 게시판 연주영상" ? "/posts5_1" : "/posts5";
            ...
            <div className="SessionWriteBoard">
              ...
              <form onSubmit={handleSubmit}>
                <div className="ImgUpload">
                  {boardType === "드럼 게시판 게시글" && imagePreview && (
                    <img src={imagePreview} alt="ImagePreview" width="300px" />
                  )}
                  {boardType === "드럼 게시판 게시글" && (
                    <label>
                      이미지 업로드:
                      <input
                        type="file"
