...
function KeyboardDetail() {
  ...
  const getDetail = async post_id => {
    ...
    try {
      const res = await axios.get(`/dailband/boards/9/${post_id}`, {
        headers: {
  ...
  const handleSubmit = async e => {
    ...
    try {
      await axios.put(`/dailband/boards/9/${post_id}`, updatedPost, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleDeleteClick = async () => {
    if (window.confirm("게시글을 삭제하시겠습니까?")) {
      try {
        await axios.delete(`/dailband//boards/9/${post_id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
