...
function Guitar() {
  ...
  const handleSubmit = async e => {
    ...
    let newPost;
    if (boardType === String("기타 게시판 연주영상")) {
      newPost = {
        title,
        link: youtubeLink,
        user_id: "이름",
            ...
            <>
              <div className="TopBoard">
                <div className="Sessionpost">
                  <h1>진정한 락스타 기타리스트 페이지!</h1>
                  {currentPosts.slice(0, 2).map(post => (
                    <Link
                      to={`/boards/6/${post.post_id}`}
            ...
            <div className="SessionWriteBoard">
              ...
              <form onSubmit={handleSubmit}>
                <div className="ImgUpload">
                  {boardType === "기타 게시판 게시글" && imagePreview && (
                    <img src={imagePreview} alt="ImagePreview" width="300px" />
                  )}
                  {boardType === "기타 게시판 게시글" && (
                    <label>
                      이미지 업로드:
                      <input
                        type="file"
                ...
                <div className="TextUpload">
                  <div>
                    ...
                    <select value={boardType} onChange={handleBoardTypeChange}>
                      <option>기타 게시판 게시글</option>
                      <option>기타 게시판 연주영상</option>
                    </select>
                  </div>
